class CaveScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.teleporter=this.physics.add.image(411, 70, 'items').setAlpha(0)
    this.addCollisions();
...
this.coveredLayer.setDepth(3)
}

  addCollisions() {
    this.physics.add.collider(this.player, this.blockedLayer);
    this.physics.add.overlap(this.player, this.teleporter, this.goOutside, null, this)
...
this.gameScene.scene.wake()
  ...
  this.tiles=this.map.addTilesetImage('caveTilesheet','caveTilesheet')
  this.backgroundLayer=this.map.createStaticLayer('background',this.tiles,0,0)
  this.blockedLayer=this.map.createStaticLayer('Blocked Layer',this.tiles,0,0)
  this.blockedLayer.setCollisionByExclusion([-1])
  this.coveredLayer=this.map.createStaticLayer('Cover Layer',this.tiles,0,0)
